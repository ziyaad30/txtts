...
class GptDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # cond_wave_clip = get_prompt_slice(audio, 10, 5, self.sample_rate, self.is_eval)
        # cond_mel = self.inj({'wav': cond_wave_clip.unsqueeze(0)})['mel']
...
class DvaeMelDataset(torch.utils.data.Dataset):
    def __init__(self, config):
        # Trying to use a different voice for DVAE
