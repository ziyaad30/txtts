...
from xtts.utils.io import load_fsspec
...
def load_audio(audiopath, sampling_rate):
    ...
    # torchaudio should choose proper backend to load audio depending on platform
...
class MyTts(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.tokenizer = VoiceBpeTokenizer(vocab_file=self.config['vocab'])
        self.gpt_use_perceiver_resampler = True
        self.gpt = None
        self.hifigan_decoder = None
        self.mel_stats_path = None
        self.gpt_batch_size = self.config['gpt_batch_size']
        self.init_models()
        self.register_buffer("mel_stats", torch.ones(80))
        print(f">>> Initialized MyTts with batch size: {self.gpt_batch_size}")

    def init_models(self):
        self.gpt = GPT(**self.config['gpt'])
        self.hifigan_decoder = HifiDecoder(**self.config['hifigan'])
    ...
    def synthesize(self, text, config, speaker_wav, **kwargs):
        ...
        return self.full_inference(text, speaker_wav, **settings)
    ...
    def inference(
        ...
        for sent in text:
            ...
            text_tokens = torch.IntTensor(self.tokenizer.encode(sent)).unsqueeze(0).to(self.device)

            assert (
                    text_tokens.shape[-1] < 402
    ...
    def inference_stream(
        ...
        for sent in text:
            ...
            while not is_end:
                ...
                if is_end or (0 < stream_chunk_size <= len(last_tokens)):
    ...
    def forward(self):
        raise NotImplementedError(
            "XTTS has a dedicated trainer, please check the XTTS docs: "
            "https://tts.readthedocs.io/en/dev/models/xtts.html#training"
        )
    ...
    @staticmethod
    def init_from_config(config):  # pylint: disable=unused-argument
        return MyTts(config)

    def eval(self):  # pylint: disable=redefined-builtin
        """Sets the model to evaluation mode. Overrides the default eval() method to also set the GPT model to eval
        mode."""
    ...
    def load_checkpoint(
        ...
        """
        Loads a checkpoint from disk and initializes the gpt model's state.

        Args:
            checkpoint_path (str, required): The path to the checkpoint file.
            model_eval (bool, optional): Whether to set the model to evaluation mode. Defaults to True.
            strict (bool, optional): Whether to strictly enforce that the keys in the checkpoint match the keys in the
        ...
        checkpoint = self.get_compatible_checkpoint_state_dict(checkpoint_path)
        ...
        if model_eval:
            ...
            self.gpt.init_gpt_for_inference(kv_cache=True, use_deepspeed=use_deepspeed)
            self.gpt.eval()
